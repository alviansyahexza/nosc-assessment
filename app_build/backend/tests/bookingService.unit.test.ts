...
class MockAppointmentRepository implements IAppointmentRepository {
  ...
  async createBooking(data: CreateAppointmentDTO): Promise<number> {
    if (data.startsAt.toISOString() === '2026-10-10T10:00:00.000Z') {
...
describe('BookingService', () => {
  ...
  it('calculates blocked time correctly including buffers', async () => {
    // Service 1: 30m duration, 5m before, 10m after. Total = 45m block.
    // Requested: 10:00 UTC
    // Expected Blocked: 09:55 UTC to 10:40 UTC
    ...
    expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({
      startsAt: new Date('2026-12-12T09:55:00.000Z'),
      endsAt: new Date('2026-12-12T10:40:00.000Z'),
      roomId: 100,
      deviceIds: [200]
    }));
  });
